...
do
    ...
    then
        ...
        curl -X POST -H 'Content-type: application/json' http://neo4j:coney@coney-neo4j:7474/db/data/transaction/commit -d '{"statements": [{"statement": "MERGE (c:Conversation {conv_id: \"id--193834919-1563808972845\", conv_title: \"Coney-demo\", conv_type: \"\", json_url: \"/opt/coney-data/Coney-demo.txt\", lang: \"en\", accessLevel: 1, status: \"saved\"});"}]}'
        break
    else
